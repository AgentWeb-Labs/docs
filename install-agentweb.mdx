...
description: "Agent-facing install flow for AgentWeb as an Agent Tool: price setup, create a scoped Dodo payment, generate repo metadata PRs, and verify agent readiness."
---
...
2. The agent calls `agentweb_install_agentweb` to produce a repo-specific install plan, expected PR files, pricing package, payment authority requirements, and verification checks.
...
- `/pricing.md`, payment authority schema references, MPP (machine payment profile) and x402 (HTTP 402 payment) discovery metadata, and receipt/verification semantics where paid workflows exist.
...
Paid setup uses scoped payment authority plus Dodo checkout, with credits granted only after a verified Dodo webhook. The full safety boundary and authority requirements live on [Agent commerce](/agent-commerce).
...
- `agentweb_create_checkout_session`: hand the signed-in customer a Starter plan checkout link (Enterprise is sales-led via [https://agentweb.us/demo](https://agentweb.us/demo)).
