...
AgentWeb supports agentic commerce through payment authority artifacts, Dodo checkout settlement, webhook-backed credits, and verification artifacts. ACP (Agentic Commerce Protocol) and UCP (Universal Commerce Protocol) are the emerging standards for agent-initiated checkout; AgentWeb publishes them as integration-target profiles so agents can detect the intended commerce bridge. The related discovery files cover MPP (a machine payment profile describing how software can pay) and x402 (the HTTP 402-based payment flow).
...
ACP/UCP-native settlement can be added behind the same authority and verification contract. The safety boundary is defined once on [Agent commerce](/agent-commerce).
