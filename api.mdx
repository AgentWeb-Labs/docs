...
Failure codes an integrator should handle: `MAP_NOT_FOUND`, `ACTION_NOT_FOUND`, `INVALID_PARAMS`, `RATE_LIMITED`, `TIMEOUT`, `UPSTREAM_ERROR`, `UPSTREAM_APP_ERROR` (transport 2xx but the payload reports failure), `AUTH_REQUIRED` (with an `error.auth` object describing the connection to make), `PAYMENT_APPROVAL_REQUIRED` and `PAYMENT_DECLINED` (with `error.payment`), `INSUFFICIENT_CREDITS` and `WORKSPACE_INACTIVE` (with `error.credits` carrying top-up or upgrade guidance). Every failure refunds the credit reservation and sets `trace_id`, a 7-day redacted trace you can quote in a support request.
