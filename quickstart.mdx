...
Cursor and Windsurf: add `https://mcp.agentweb.us/mcp` as a remote MCP server by URL (Streamable HTTP; no catalog entry needed). Any other MCP client takes the same URL. Per-client steps: [install.md](https://www.agentweb.us/install.md).
...
Call `execute` (or `POST /api/execute` over REST) with the domain, action, and declared params. Optionally pass a stable `task_id` (MCP) or `Idempotency-Key` header (REST); a retried call with the same key maps to the same billing task and is never charged twice:
