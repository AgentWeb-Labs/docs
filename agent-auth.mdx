...
Some target sites use cookie-backed connections managed by AgentWeb (for example the partner site Hermes, tryhermes.dev). For these providers, agents should call `agentweb_auth_status` or `agentweb_connection_status` first. If the provider is not connected, call `agentweb_start_connection`, send the human to the returned `connect_url`, then call `agentweb_poll_connection` until it returns `connected=true`.
